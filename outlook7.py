...
def mail_adjuntos(mail_carpeta: str,
    ...
    for msg in mensajes:
        ...
        filtro_asunto = asunto_mail.lower() in asunto if asunto_mail else True
        ...
        if condicion_mail:
            ...
            for attachment in msg.Attachments:
                ...
                if nombre_archivo.suffix.lower() in extensiones_permitidas and filtro_nombre:
                    ...
                    nombre_final = None
                    for clave, remitentes in archivo_rutas.items():
                        if clave.lower() in nombre_archivo.stem.lower():
                            remitente_info = remitentes.get(remitente)
                            if remitente_info:
                                destino_especifico = Path(remitente_info['ruta'])
                                nombre_final = remitente_info['nombre_final']
                                break
    ...
    mensaje_imprimir(f'\n{mensajes_total} MENSAJES TOTALES EN {mail_carpeta} CON LAS CONDICIONES:'
                     f'\nCORREO: {mail_cuenta}\nCARPETA: {mail_carpeta}\nASUNTO MAIL: {asunto_mail or "TODOS"}\nNOMBRE ARCHIVO: {adjunto_nombre or "TODOS"}'
...
archivo_rutas = {
    'stock_diario': {
        'mailaccount@fcm-eu.com': {
            'ruta': Path(r'C:\Informes\Stock\FCM'),
            'nombre_final': 'stock_fcm'
        },
        'ejemplo@ejemplo.com': {
            'ruta': Path(r'C:\Informes\Stock\EJEMPLO'),
            'nombre_final': 'stock_ejemplo'
        }
    },
    'ventas_mensuales': {
        'mailaccount@fcm-eu.com': {
            'ruta': Path(r'C:\Informes\Ventas\FCM'),
            'nombre_final': 'ventas_fcm'
        }
    }
}
